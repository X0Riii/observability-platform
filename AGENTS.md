...
16. **Kafka broker IPv6** — librdkafka resolves `localhost` to `::1` first; IPv6 disabled at kernel level causes "Network is unreachable". Fixed by setting `broker.address.family=v4` on all Kafka clients and changing default broker from `localhost:9093` to `127.0.0.1:9092`.
...
- **Rust API Kafka** — Port was `9093` in default config, corrected to `9092`. Consumer subscribes to topics. `broker.address.family=v4` required when IPv6 is disabled.
- **Build workaround** — Source copied to `/tmp/opencode/api-rust/` (no spaces), `CARGO_TARGET_DIR=/tmp/opencode/rust-target`.

## Next Steps
1. Verify Dashboard at http://localhost:3000 renders live data.
2. Add `cdp_request_id` column for proper FK join between requests and responses.
